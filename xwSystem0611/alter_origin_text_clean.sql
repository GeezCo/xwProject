...
-- 删除可能存在的备份列
ALTER TABLE origin_text DROP COLUMN IF EXISTS times_backup;
...
SET @sql = IF(@col_exists = 0,
    ...
    'SELECT ''briefTypeName already exists'' AS message');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- 3. 新增 sendUnitName 字段（如果不存在）
SET @col_exists = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
                   WHERE TABLE_SCHEMA = 'uygur_project'
                   AND TABLE_NAME = 'origin_text'
...
SET @sql = IF(@col_exists = 0,
    ...
    'SELECT ''sendUnitName already exists'' AS message');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = IF(@idx_exists = 0,
    ...
    'SELECT ''idx_times already exists'' AS message');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = IF(@idx_exists = 0,
    ...
    'SELECT ''idx_briefTypeName already exists'' AS message');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = IF(@idx_exists = 0,
    ...
    'SELECT ''idx_sendUnitName already exists'' AS message');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
WHERE TABLE_SCHEMA = 'uygur_project'
  AND TABLE_NAME = 'origin_text'
  AND COLUMN_NAME IN ('times', 'briefTypeName', 'sendUnitName', 'times_backup')
