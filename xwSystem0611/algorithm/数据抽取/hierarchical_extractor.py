...
# 导入分类服务客户端（用于智能标签生成）
from classify_client import classify_events, extract_entities_from_events
...
class HierarchicalEventExtractor:
    ...
    def extract_all(self, text: str) -> Dict[str, Any]:
        ...
        # 4.5 对事件进行分类标签（调用RexUniNlu分类服务）
        print(f"正在进行分类标签...")
        unique_events = classify_events(unique_events)

        # 4.6 提取并汇总实体
        print(f"正在提取实体...")
        entities = extract_entities_from_events(unique_events)
