...
{text}"""
        ...
        for attempt in range(retry_count):
            try:
                ...
                response = self.client.chat.completions.create(
                    ...
                    temperature=CONFIG['llm'].get('temperature', 0.1),  # 低温度使输出更稳定
                    max_tokens=CONFIG['llm'].get('max_tokens', 16000)  # 最大输出token数
                )
    ...
    def parse_llm_response(self, response: Dict, paragraph: str) -> List[Dict]:
        ...
        try:
            ...
            try:
                ...
                # 为缺失time的事件补充时间
                events = self._fix_missing_time(events, paragraph)
                return events
    ...
    def _fix_missing_time(self, events: List[Dict], paragraph: str) -> List[Dict]:
        """
        为缺失time字段的事件补充时间，并规范化已有时间格式

        处理：
        1. 清理time字段中的多余空格（如"2008 年 7 月 25 日" → "2008年7月25日"）
        2. 为缺失time的事件从原文提取时间
        3. 确保所有字段都有默认值
        ...
        for event in events:
            ...
            if event.get('time') and event['time'] not in ['未抽取', '', None]:
                ...
                normalized_time = normalized_time.strip()
                if normalized_time:
                    event['time'] = normalized_time

            # 只有当time为空或"未抽取"时才尝试提取
            if event.get('time') in ['', '未抽取', None]:
                # 尝试从多个来源提取时间
                sources = [
                    event.get('original_text', ''),
                    paragraph,
                    event.get('action', '')  # 有时action包含时间
                ]
                        ...
                        for pattern in time_patterns:
                            ...
                            if match:
                                ...
                                if len(groups) >= 3 and groups[0]:
                                    ...
                                    extracted_time = f"{year}年{month}月{day}日"
                                elif len(groups) == 2 and groups[0]:
                                    year, month = groups[0], groups[1]
                                    extracted_time = f"{year}年{month}月"
                                if extracted_time:
                                    break
                        if extracted_time:
                            break
    ...
    def _parse_event_manually(self, event_str: str) -> Optional[Dict]:
        ...
        Returns:
            Optional[Dict]: 解析成功的事件字典，失败返回None
        """
        event = {
            'time': '未抽取',
            'location': [],
        ...
        # 提取subject（可能是数组或字符串）
        subject_match = re.search(r'"subject"\s*:\s*(\[.*?\]|"[^"]*")', event_str)
        if subject_match:
            subject_value = subject_match.group(1)
            try:
                if subject_value.startswith('['):
                    # 清理并解析数组
                    subject_value = self._fix_json_format(subject_value)
                    parsed = json.loads(subject_value)
                    if parsed:
                        event['subject'] = parsed
                else:
                    val = subject_value.strip('"')
                    if val:
                        event['subject'] = [val]
            except:
                # 强制提取数组中的字符串
                items = re.findall(r'"([^"]*)"', subject_value)
                if items:
                    event['subject'] = items
        ...
        # 提取location（数组）
        location_match = re.search(r'"location"\s*:\s*(\[.*?\])', event_str)
        if location_match:
            try:
                loc_value = self._fix_json_format(location_match.group(1))
                parsed = json.loads(loc_value)
                if parsed:
                    event['location'] = parsed
            except:
                items = re.findall(r'"([^"]*)"', location_match.group(1))
                if items:
        ...
        if original_match:
            ...
            if original.strip():
                event['original_text'] = original.strip()
        ...
        if time_match:
            ...
            if time_val.strip():
                event['time'] = time_val.strip()

        # subject为空时记为无效事件
        if not event['subject'] or event['subject'] == []:
            return None
    ...
    def _force_extract_events(self, content: str, paragraph: str) -> List[Dict]:
        """
        强制从内容中提取所有事件信息（最后的兜底方法）

        使用宽松的正则匹配提取所有可能的事件字段
        ...
        for block in brace_blocks:
            ...
            if event and (event.get('subject') or event.get('action')):
                events.append(event)
    ...
    def _fix_missing_time(self, events: List[Dict], paragraph: str) -> List[Dict]:
        ...
        Returns:
            List[Dict]: 补充时间后的事件列表
        """
        for event in events:
            if not event.get('time'):
                # 尝试从original_text提取时间
                original_text = event.get('original_text', '')
                time_pattern = r'(\d{4})[年\s\-/]*(\d{1,2})[月\s\-/]*(\d{1,2})[日]?'
                match = re.search(time_pattern, original_text)
                if match:
                    year, month, day = match.groups()
                    event['time'] = f"{year}年{month}月{day}日"
                else:
                    # 尝试从段落提取时间
                    match = re.search(time_pattern, paragraph)
                    if match:
                        year, month, day = match.groups()
                        event['time'] = f"{year}年{month}月{day}日"
                    else:
        ...
        for i, char in enumerate(events_str):
            if char == '{':
                ...
                if brace_count == 0 and in_event:
                    ...
                    event_str = events_str[start_idx:i+1]
                    # 修复中文引号
                    event_str = event_str.replace('"', '"').replace('"', '"').replace('：', ':')
                    try:
                        event = json.loads(event_str)
                        events.append(event)
                    except json.JSONDecodeError:
    ...
    def _parse_event_manually(self, event_str: str) -> Optional[Dict]:
        ...
        if subject_match:
            try:
                event['subject'] = json.loads('[' + subject_match.group(1) + ']')
            except:
                event['subject'] = []
        ...
        if location_match:
            try:
                event['location'] = json.loads('[' + location_match.group(1) + ']')
            except:
                event['location'] = []
        ...
        original_match = re.search(r'"original_text"\s*:\s*"([^"]*)"', event_str)
        if original_match:
            event['original_text'] = original_match.group(1)
        ...
        # subject为空时记为无效事件
        if not event.get('subject') or event.get('subject') == []:
            return None
